...
set<int>::const_iterator FindNearestElement(const set<int>& numbers, int border){
	set<int>::const_iterator el = begin(numbers);;
	if (numbers.empty()){return el;}
	int pr = *el;
	for (auto it = ++begin(numbers); it != end(numbers); it++){
		if (border > *it) {pr = *it; el = it; continue;}
		else {
			if (*it - border < border - pr){el = it; break;}
			else {break;}
		}
	}
	return el;
...
/*
set<int>::const_iterator FindNearestElement1(const set<int>& numbers, int border) {
